...
struct cfg {
    ...
    inline static reflection::source_location location{};
    inline static bool                        wip{};

#if defined(_MSC_VER)
    static inline int          largc = __argc;
    inline static const char** largv = const_cast<const char**>(__argv);
#else
    static inline int          largc = 0;
    inline static const char** largv = nullptr;
#endif

    static inline std::string executable_name = "unknown executable";
    inline static std::string query_pattern;                 // <- done
    inline static bool        invert_query_pattern = false;  // <- done
    inline static std::string query_regex_pattern;           // <- done
    inline static bool        show_help             = false; // <- done
    inline static bool        show_tests            = false; // <- done
    inline static bool        list_tags             = false; // <- done
    inline static bool        show_successful_tests = false; // <- done
    inline static std::string output_filename;
    inline static std::string use_reporter = "console"; // <- done
    inline static std::string suite_name;
    inline static bool        abort_early            = false;                                   // <- done
    inline static std::size_t abort_after_n_failures = std::numeric_limits<std::size_t>::max(); // <- done
    inline static bool        show_duration          = false;                                   // <- done
    inline static std::size_t show_min_duration      = 0;
    inline static std::string input_filename;
    inline static bool        show_test_names   = false; // <- done
    inline static bool        show_reporters    = false; // <- done
    inline static std::string sort_order        = "decl";
    inline static std::size_t rnd_seed          = 0;     // 0: use time
    inline static std::string use_colour        = "yes"; // <- done
    inline static bool        show_lib_identity = false; // <- done
    inline static std::string wait_for_keypress = "never";

    inline static const std::vector<option> options = {
        // clang-format off
...
template <typename T>
struct is_stream_insertable<T, std::void_t<decltype(std::declval<std::ostream&>() << detail::get(std::declval<T>()))> >
    : std::true_type {};
template <typename T>
...
class reporter_junit {
  ...
  protected:
    ...
    void print_junit_summary(std::ostream& stream) {
        ...
        auto   suite_time = [](const auto& suite_result) {
            std::int64_t time_ms =
  ...
  public:
    ...
    auto on(events::test<Ts...> test) {
        ...
        if (detail::cfg::list_tags) {
            std::for_each(
                test.tag.cbegin(), test.tag.cend(), [](const auto& tag) { std::cout << "tag: " << tag << std::endl; });
            return;
        }
        ...
        if (!detail::cfg::query_pattern.empty()) {
            static const auto regex   = detail::cfg::query_regex_pattern;
        ...
        if (filter_(level_, path_)) {
            ...
            } else { // N.B. prev. only root-level tests were signaled on finish
    ...
    return [f, t, unique_name](std::string_view type, std::string_view name) {
        ...
        apply(
            [=, &counter](const auto&... args) {
                (detail::on<F>(events::test<F, Ts>{.type = type,
