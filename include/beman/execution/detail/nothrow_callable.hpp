...
#include <beman/execution/detail/callable.hpp>
    ...
    namespace beman::execution::detail { template <typename Fun, typename... Args> concept nothrow_callable = ::beman::execution::detail::callable <Fun, Args...> && requires(Fun && fun, Args &&...args) { { ::std::forward <Fun>(fun)(::std::forward <Args>(args)...) } noexcept;
};
